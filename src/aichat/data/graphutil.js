...
function autoscale_and_length(data, width, height) {
    ...
    // 0.6931471805599453 / ...
    var exp = Math.LN2 / Math.log((x_max * 1.1 - x_min * 0.9) / (x_mid * 1.05 - x_min * 0.95));
    console.log('min, mid, max: ' + [x_min, x_mid, x_max] + ' gives exponent: ' + exp);
    pow_scale = d3.scale.pow().exponent(exp).domain([x_min * .1, x_max * 10]).range([0.0, Math.pow(1.0-.15,1/4.)]);
    ...
    console.log("length: " + length + " for nedges, nnodes: " + N_edges + ", " +  N_nodes + " edges and nodes");
